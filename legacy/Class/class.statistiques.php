...
function statistiques1($nom, $tab, $debutAlpha, $finAlpha, $separateur, $nbJours, $debut, $fin): array
{
    ...
    if ($nom=="agent") {
        $lignes[]=implode($separateur, array("Nom","Prénom","Nombre de samedis","Dates","Heures"));
    } else {
        $lignes[]=implode($separateur, array(ucfirst($nom),"Nombre de samedis","Dates","Heures"));
    }
    foreach ($tab as $elem) {
    ...
    if ($GLOBALS['config']['Dimanche']) {
        ...
        $lignes[]="Dimanches";
        if ($nom=="agent") {
            $lignes[]=implode($separateur, array("Nom","Prénom","Nombre de dimanches","Dates","Heures"));
        } else {
            $lignes[]=implode($separateur, array(ucfirst($nom),"Nombre de dimanches","Dates","Heures"));
        }
        foreach ($tab as $elem) {
    ...
    $lignes[]="Jours fériés";
    if ($nom=="agent") {
        $lignes[]=implode($separateur, array("Nom","Prénom","Nombre de jours feriés","Dates","Heures"));
    } else {
        $lignes[]=implode($separateur, array(ucfirst($nom),"Nombre de jours feriés","Dates","Heures"));
    }
    foreach ($tab as $elem) {
    ...
    $lignes[]="Absences";
    if ($nom=="agent") {
        $lignes[]=implode($separateur, array("Nom","Prénom","Heures d'absences","Dates","Heures"));
    } else {
        $lignes[]=implode($separateur, array(ucfirst($nom),"Heures d'absences","Dates","Heures"));
    }
    foreach ($tab as $elem) {
    ...
    foreach ($heures as $k1 => $v1) {
        ...
        if ($nom=="agent") {
            $lignes[]=implode($separateur, array("Nom","Prénom","Nombre de $hres","Dates"));
        } else {
            $lignes[]=implode($separateur, array(ucfirst($nom),"Nombre de $hres","Dates","Nombre à cette date"));
        }
